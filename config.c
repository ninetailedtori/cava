...
static void LoadFileInResource(int name, int type, DWORD *size,
                               const char **data) {
...
int default_theme_data[NUMBER_OF_THEMES] = {IDR_SOLARIZED_DARK_THEME,
                                            IDR_TRICOLOR_THEME};
#else
...
INCTXT(winamp_line_style_spectrum,
...
const char *default_theme_data[NUMBER_OF_THEMES] = {gsolarized_darkData,
                                                    gtricolorData};
#endif // _WIN32
...
const char *default_theme_name[NUMBER_OF_THEMES] = {"solarized_dark",
                                                    "tricolor"};
...
void write_errorf(void *err, const char *fmt, ...) {
  ...
  va_list args;
  va_start(args, fmt);
  error->length += vsnprintf((char *)error->message + error->length,
                             MAX_ERROR_LEN - error->length, fmt, args);
  va_end(args);
}
...
bool validate_colors(void *params, void *err) {
  ...
  if (!validate_color(p->color, p, error)) {
    write_errorf(
        error,
        "The value for 'foreground' is invalid. It can be either one of the 7 "
  ...
  if (!validate_color(p->bcolor, p, error)) {
    write_errorf(
        error,
        "The value for 'background' is invalid. It can be either one of the 7 "
        "named colors or a HTML color of the form '#xxxxxx'.\n");
    return false;
  }
...
bool validate_config(struct config_params *p, struct error_s *error) {
  ...
  if (strcmp(outputMethod, "ncurses") == 0) {
    ...
    write_errorf(
        error,
        "cava was built without ncurses support, install ncursesw dev files "
...
#ifndef SDL
    write_errorf(error,
                 "cava was built without sdl support, install sdl dev files "
                 "and run make clean && ./configure && make again\n");
    return false;
#endif
  }
  if (strcmp(outputMethod, "sdl_glsl") == 0) {
    ...
    write_errorf(
        error,
        "cava was built without opengl support, install opengl dev files "
        "and run make clean && ./configure && make again\n");
    return false;
#endif
  }
...
#endif
    write_errorf(
        error, "output method %s is not supported, supported methods are: %s\n",
  ...
  if (strcmp(orientation, "horizontal") == 0) {
    if (p->output != OUTPUT_NONCURSES) {
      write_errorf(error,
                   "only noncurses output supports horizontal orientation\n");
      return false;
    }
  ...
  if (strcmp(orientation, "vertical") == 0) {
    if (p->output != OUTPUT_NONCURSES) {
      write_errorf(error,
                   "only noncurses output supports vertical orientation\n");
      return false;
    }
    p->orientation = ORIENT_SPLIT_V;
    p->left_bottom = 1; // this setting is used to flip channels in horizontal
                        // split mode, makes no sense here. forcing this to 1
                        // puts left on left and right on right
  }
  ...
  if ((p->orientation == ORIENT_LEFT || p->orientation == ORIENT_RIGHT) &&
    ...
    write_errorf(
        error,
        "only noncurses, ncurses and sdl supports left/right orientation\n");
    return false;
  }
  if ((p->orientation == ORIENT_TOP) &&
      !(p->output == OUTPUT_SDL || p->output == OUTPUT_NCURSES ||
        p->output == OUTPUT_NONCURSES)) {
    write_errorf(error,
                 "only noncurses, ncurses and sdl supports top orientation\n");
    return false;
  }
  ...
  if (p->max_height > 1.0) {
    write_errorf(
        error, "max_height is defined in percent and can't be more than 100\n");
    return false;
...
bool load_config(char configPath[PATH_MAX], struct config_params *p,
  ...
  if (configPath[0] == '\0') {
    ...
    if (fp) {
      ...
      if (ftell(fp) == 0) {
        ...
        LoadFileInResource(IDR_CONFIG_FILE, TEXTFILE, &gConfigFileSize,
                           &gConfigFileData);
        gConfigFileSize += 1;
#endif
...
#ifndef _WIN32
        fwrite(default_shader_data[i], strlen(default_shader_data[i]),
               sizeof(char), fp);
#else
        DWORD shaderDataSize = 0;
        const char *shaderData = NULL;
        LoadFileInResource(default_shader_data[i], TEXTFILE, &shaderDataSize,
                           &shaderData);
...
#ifndef _WIN32
        fwrite(default_theme_data[i], strlen(default_theme_data[i]),
               sizeof(char), fp);
#else
        DWORD themeDataSize = 0;
        const char *themeData = NULL;
        LoadFileInResource(default_theme_data[i], TEXTFILE, &themeDataSize,
                           &themeData);
  ...
  orientation =
      strdup(iniparser_getstring(ini, "output:orientation", "bottom"));
  ...
  p->noise_reduction =
      iniparser_getdouble(ini, "smoothing:noise_reduction", 77);
  ...
  monoOption =
      strdup(iniparser_getstring(ini, "output:mono_option", "average"));
  ...
  p->raw_target =
      strdup(iniparser_getstring(ini, "output:raw_target", "/dev/stdout"));
  p->data_format =
      strdup(iniparser_getstring(ini, "output:data_format", "binary"));
  ...
  blendDirection =
      strdup(iniparser_getstring(ini, "color:blend_direction", "up"));
  ...
  if (strcmp(outputMethod, "sdl") == 0 ||
      strcmp(outputMethod, "sdl_glsl") == 0) {
  ...
  p->continuous_rendering =
      iniparser_getint(ini, "output:continuous_rendering", 0);
  ...
  p->show_idle_bar_heads =
      iniparser_getint(ini, "output:show_idle_bar_heads", 1);
  ...
  vertexShader = strdup(
      iniparser_getstring(ini, "output:vertex_shader", "pass_through.vert"));
  fragmentShader = strdup(
      iniparser_getstring(ini, "output:fragment_shader", "bar_spectrum.frag"));
  ...
  char *input_method_name = strdup(iniparser_getstring(
      ini, "input:method", input_method_names[default_input]));
  ...
  case INPUT_ALSA:
    p->audio_source =
        strdup(iniparser_getstring(ini, "input:source", "hw:Loopback,1"));
    break;
#endif
  case INPUT_FIFO:
    p->audio_source =
        strdup(iniparser_getstring(ini, "input:source", "/tmp/mpd.fifo"));
    break;
#ifdef PULSE
  case INPUT_PULSE:
  ...
  case INPUT_SNDIO:
    p->audio_source =
        strdup(iniparser_getstring(ini, "input:source", SIO_DEVANY));
    break;
#endif
#ifdef OSS
  case INPUT_OSS:
    p->audio_source =
        strdup(iniparser_getstring(ini, "input:source", "/dev/dsp"));
    break;
#endif
#ifdef JACK
  case INPUT_JACK:
    p->audio_source =
        strdup(iniparser_getstring(ini, "input:source", "default"));
    break;
#endif
  case INPUT_SHMEM:
    p->audio_source = strdup(iniparser_getstring(
        ini, "input:source", "/squeezelite-00:00:00:00:00:00"));
    break;
#ifdef PORTAUDIO
  ...
  iniparser_freedict(ini);
#else
  GetPrivateProfileString("output", "method", "noncurses", outputMethod, 16,
                          configPath);

  p->waves = GetPrivateProfileInt("smoothing", "waves", 0, configPath);
  p->monstercat =
      GetPrivateProfileInt("smoothing", "monstercat", 0, configPath);

  p->noise_reduction =
      GetPrivateProfileInt("smoothing", "noise_reduction", 70, configPath);
  GetPrivateProfileString("output", "xaxis", "none", xaxisScale, 16,
                          configPath);
  GetPrivateProfileString("output", "orientation", "bottom", orientation, 16,
                          configPath);
  GetPrivateProfileString("color", "blend_orientation", "up", orientation, 16,
                          configPath);
  ...
  p->bar_spacing =
      GetPrivateProfileInt("general", "bar_spacing", 1, configPath);
  ...
  p->lower_cut_off =
      GetPrivateProfileInt("general", "lower_cutoff_freq", 50, configPath);
  p->upper_cut_off =
      GetPrivateProfileInt("general", "higher_cutoff_freq", 10000, configPath);
  p->sleep_timer =
      GetPrivateProfileInt("general", "sleep_timer", 0, configPath);
  p->live_config =
      GetPrivateProfileInt("general", "live-config", 0, configPath);
  p->max_height =
      GetPrivateProfileInt("general", "max_height", 100, configPath) / 100.0;
  p->center_align =
      GetPrivateProfileInt("general", "center_align", 1, configPath);

  GetPrivateProfileString("output", "channels", "stereo", channels, 16,
                          configPath);
  GetPrivateProfileString("output", "mono_option", "average", monoOption, 16,
                          configPath);
  p->reverse = GetPrivateProfileInt("output", "reverse", 0, configPath);
  GetPrivateProfileString("output", "raw_target", "/dev/stdout", p->raw_target,
                          64, configPath);
  GetPrivateProfileString("output", "data_format", "binary", p->data_format, 16,
                          configPath);
  p->bar_delim =
      (char)GetPrivateProfileInt("output", "bar_delimiter", 59, configPath);
  p->frame_delim =
      (char)GetPrivateProfileInt("output", "frame_delimiter", 10, configPath);
  p->ascii_range =
      GetPrivateProfileInt("output", "ascii_max_range", 1000, configPath);
  ...
  p->sync_updates =
      GetPrivateProfileInt("output", "synchronized_sync", 0, configPath);
  p->show_idle_bar_heads =
      GetPrivateProfileInt("output", "show_idle_bar_heads", 1, configPath);
  ...
  while (1) {
    ...
    GetPrivateProfileString("eq", keyNum, "NOT FOUND", eqResult,
                            sizeof(eqResult), configPath);
    if (!strcmp(eqResult, "NOT FOUND")) {
      break;
    } else {
      double *oldPtr = p->userEQ;
      p->userEQ =
          (double *)realloc(p->userEQ, sizeof(double) * (p->userEQ_keys + 1));
      if (p->userEQ == NULL) {
      ...
      if (endptr == eqResult) {
        write_errorf(error,
                     "Invalid string to double conversion, %d : \"%s\" \n",
  ...
  if (p->input != INPUT_WINSCAP) {
    write_errorf(error,
                 "on windows changing input method is not supported, "
                 "simply leave the input method setting commented out\n");
    free(cava_config_home);
    free_config(p);
    return false;
  }
  GetPrivateProfileString("input", "source", "auto", p->audio_source, 64,
                          configPath);
  ...
  if (strcmp(outputMethod, "sdl") == 0 ||
    ...
    p->bar_spacing =
        GetPrivateProfileInt("general", "bar_spacing", 5, configPath);
    if (strcmp(p->color, "default") == 0) {
      GetPrivateProfileString("color", "foreground", "#33cccc", p->color, 9,
                              configPath);
    }
    if (strcmp(p->bcolor, "default") == 0) {
      GetPrivateProfileString("color", "background", "#111111", p->bcolor, 9,
                              configPath);
    }
  }

  p->continuous_rendering =
      GetPrivateProfileInt("output", "continuous_rendering", 0, configPath);

  p->sdl_glsl_gain =
      GetPrivateProfileInt("output", "sdl_glsl_gain", 100, configPath);

  GetPrivateProfileString("output", "vertex_shader", "pass_through.vert",
                          vertexShader, 64, configPath);
  GetPrivateProfileString("output", "fragment_shader", "bar_spectrum.frag",
                          fragmentShader, 64, configPath);
  ...
  sprintf(p->fragment_shader, "%s/shaders/%s", cava_config_home,
          fragmentShader);
...
bool load_colors(char *themeFile, struct config_params *p,
                 struct error_s *error) {
  free_colors(p);
  ...
  p->gradient_colors[0] =
      strdup(iniparser_getstring(ini, "color:gradient_color_1", "not_set"));
  p->gradient_colors[1] =
      strdup(iniparser_getstring(ini, "color:gradient_color_2", "not_set"));
  p->gradient_colors[2] =
      strdup(iniparser_getstring(ini, "color:gradient_color_3", "not_set"));
  p->gradient_colors[3] =
      strdup(iniparser_getstring(ini, "color:gradient_color_4", "not_set"));
  p->gradient_colors[4] =
      strdup(iniparser_getstring(ini, "color:gradient_color_5", "not_set"));
  p->gradient_colors[5] =
      strdup(iniparser_getstring(ini, "color:gradient_color_6", "not_set"));
  p->gradient_colors[6] =
      strdup(iniparser_getstring(ini, "color:gradient_color_7", "not_set"));
  p->gradient_colors[7] =
      strdup(iniparser_getstring(ini, "color:gradient_color_8", "not_set"));

  p->horizontal_gradient =
      iniparser_getint(ini, "color:horizontal_gradient", 0);

  p->horizontal_gradient_colors[0] = strdup(
      iniparser_getstring(ini, "color:horizontal_gradient_color_1", "not_set"));
  p->horizontal_gradient_colors[1] = strdup(
      iniparser_getstring(ini, "color:horizontal_gradient_color_2", "not_set"));
  p->horizontal_gradient_colors[2] = strdup(
      iniparser_getstring(ini, "color:horizontal_gradient_color_3", "not_set"));
  p->horizontal_gradient_colors[3] = strdup(
      iniparser_getstring(ini, "color:horizontal_gradient_color_4", "not_set"));
  p->horizontal_gradient_colors[4] = strdup(
      iniparser_getstring(ini, "color:horizontal_gradient_color_5", "not_set"));
  p->horizontal_gradient_colors[5] = strdup(
      iniparser_getstring(ini, "color:horizontal_gradient_color_6", "not_set"));
  p->horizontal_gradient_colors[6] = strdup(
      iniparser_getstring(ini, "color:horizontal_gradient_color_7", "not_set"));
  p->horizontal_gradient_colors[7] = strdup(
      iniparser_getstring(ini, "color:horizontal_gradient_color_8", "not_set"));

  iniparser_freedict(ini);
#else
  GetPrivateProfileString("color", "foreground", "default", p->color, 9,
                          themeFile);
  GetPrivateProfileString("color", "background", "default", p->bcolor, 9,
                          themeFile);
  ...
  GetPrivateProfileString("color", "gradient_color_1", "not_set",
                          p->gradient_colors[0], 9, themeFile);
  GetPrivateProfileString("color", "gradient_color_2", "not_set",
                          p->gradient_colors[1], 9, themeFile);
  GetPrivateProfileString("color", "gradient_color_3", "not_set",
                          p->gradient_colors[2], 9, themeFile);
  GetPrivateProfileString("color", "gradient_color_4", "not_set",
                          p->gradient_colors[3], 9, themeFile);
  GetPrivateProfileString("color", "gradient_color_5", "not_set",
                          p->gradient_colors[4], 9, themeFile);
  GetPrivateProfileString("color", "gradient_color_6", "not_set",
                          p->gradient_colors[5], 9, themeFile);
  GetPrivateProfileString("color", "gradient_color_7", "not_set",
                          p->gradient_colors[6], 9, themeFile);
  GetPrivateProfileString("color", "gradient_color_8", "not_set",
                          p->gradient_colors[7], 9, themeFile);

  p->horizontal_gradient =
      GetPrivateProfileInt("color", "horizontal_gradient", 0, themeFile);
...
bool get_themeFile(char configPath[PATH_MAX], struct config_params *p,
                   char *cava_config_home, struct error_s *error,
                   char **themeFile) {
  FILE *fp;
  char *cava_cfg_home;
