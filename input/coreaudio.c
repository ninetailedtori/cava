...
static int get_channel_count(AudioDeviceID device,
  ...
  OSStatus status =
      AudioObjectGetPropertyDataSize(device, &address, 0, NULL, &property_size);
  ...
  status = AudioObjectGetPropertyData(device, &address, 0, NULL, &property_size,
                                      buffer_list);
  if (status != noErr) {
    free(buffer_list);
    return 0;
...
static bool copy_device_name(AudioDeviceID device, char *name,
                             size_t name_size) {
  AudioObjectPropertyAddress address = {
  ...
  OSStatus status = AudioObjectGetPropertyData(device, &address, 0, NULL,
                                               &property_size, &cf_name);
  if (status != noErr || cf_name == NULL) {
    return false;
  }

  Boolean ok = CFStringGetCString(cf_name, name, (CFIndex)name_size,
                                  kCFStringEncodingUTF8);
  CFRelease(cf_name);
  return ok == true;
}

static bool get_devices(AudioDeviceID **devices_out, UInt32 *count_out,
  ...
  OSStatus status = AudioObjectGetPropertyDataSize(
      kAudioObjectSystemObject, &address, 0, NULL, &property_size);
  if (status != noErr || property_size == 0) {
    return false;
  }
  ...
  status = AudioObjectGetPropertyData(kAudioObjectSystemObject, &address, 0,
                                      NULL, &property_size, all_devices);
  if (status != noErr) {
    free(all_devices);
    return false;
  }
  ...
  AudioDeviceID *devices =
      (AudioDeviceID *)malloc(all_count * sizeof(AudioDeviceID));
  if (devices == NULL) {
    free(all_devices);
    return false;
  }
  ...
  for (UInt32 i = 0; i < all_count; i++) {
    int in_channels =
        get_channel_count(all_devices[i], kAudioDevicePropertyScopeInput);
    int out_channels =
        get_channel_count(all_devices[i], kAudioDevicePropertyScopeOutput);
    if ((include_input && in_channels > 0) ||
        (include_output && out_channels > 0)) {
...
static bool get_default_device(UInt32 selector, AudioDeviceID *device_out) {
  ...
  OSStatus status = AudioObjectGetPropertyData(
      kAudioObjectSystemObject, &address, 0, NULL, &size, device_out);
...
static bool select_input_device(const char *source, AudioDeviceID *device_out) {
  if (strcmp(source, "auto") == 0 || strcmp(source, "auto_output") == 0 ||
    ...
    return get_default_device(kAudioHardwarePropertyDefaultOutputDevice,
                              device_out);
  }

  if (strcmp(source, "auto_input") == 0 ||
      strcmp(source, "default_input") == 0) {
    AudioObjectPropertyAddress address = {
...
static OSStatus input_callback(void *inRefCon,
  ...
  if (bytes_needed > state->buffer_size) {
    unsigned char *new_buffer =
        (unsigned char *)realloc(state->buffer, bytes_needed);
    if (new_buffer == NULL) {
  ...
  OSStatus status = AudioUnitRender(state->audio_unit, ioActionFlags,
                                    inTimeStamp, 1, inNumberFrames, &input);
  if (status != noErr) {
    return status;
  }

  write_to_cava_input_buffers(
      (int16_t)(inNumberFrames * state->audio->channels), state->buffer,
      state->audio);
...
static bool configure_stream_format(struct coreaudio_state *state) {
  ...
  if (audio->format != 8 && audio->format != 16 && audio->format != 24 &&
      audio->format != 32) {
    audio->format = 16;
  }
  ...
  OSStatus status = AudioUnitSetProperty(
      state->audio_unit, kAudioUnitProperty_StreamFormat,
...
void *input_coreaudio(void *audiodata) {
  ...
  if (strcmp(audio->source, "list") == 0) {
    list_input_devices();
    printf("Use 'source = auto' for the default output device, 'auto_input' "
           "for the default "
           "input device, an index from this list, or an exact device name.\n");
#ifdef COREAUDIO_TAP
    printf("Use 'source = tap' (or 'tap_mono') to capture system output "
           "directly via CoreAudio "
           "taps (macOS 14.2+).\n");
#endif
    exit(EXIT_SUCCESS);
  }
  ...
  if (!select_input_device(audio->source, &device)) {
    fprintf(stderr,
            "Error: could not find Core Audio device '%s'. Use source='list' "
            "to inspect "
            "available devices.\n",
            audio->source);
    exit(EXIT_FAILURE);
  ...
  int device_input_channels =
      get_channel_count(device, kAudioDevicePropertyScopeInput);
  if (device_input_channels == 0) {
    if (device_has_output_channels(device)) {
      fprintf(stderr, "Error: selected Core Audio device has output channels "
                      "but no capture stream. "
                      "On macOS, output mix capture still requires a "
                      "loopback-capable device.\n");
    } else {
      fprintf(stderr,
              "Error: selected Core Audio device has no input channels.\n");
    }
    exit(EXIT_FAILURE);
  }
  ...
  if (AudioObjectGetPropertyData(device, &nominal_rate_address, 0, NULL,
                                 &nominal_size, &nominal_rate) == noErr &&
      nominal_rate > 0) {
  ...
  if (status != noErr) {
    fprintf(stderr,
            "Error: could not instantiate Core Audio unit (status %d)\n",
            (int)status);
    exit(EXIT_FAILURE);
  }
  ...
  status = AudioUnitSetProperty(
      state.audio_unit, kAudioOutputUnitProperty_EnableIO,
  ...
  if (status != noErr) {
    fprintf(stderr,
            "Error: could not configure Core Audio IO mode (status %d)\n",
            (int)status);
  ...
  status = AudioUnitSetProperty(
      state.audio_unit, kAudioOutputUnitProperty_CurrentDevice,
      kAudioUnitScope_Global, 0, &device, sizeof(device));
  if (status != noErr) {
    fprintf(stderr,
            "Error: could not select Core Audio input device (status %d)\n",
            (int)status);
    AudioComponentInstanceDispose(state.audio_unit);
    exit(EXIT_FAILURE);
  }
  ...
  status = AudioUnitSetProperty(
      state.audio_unit, kAudioOutputUnitProperty_SetInputCallback,
      kAudioUnitScope_Global, 0, &callback, sizeof(callback));
  if (status != noErr) {
    fprintf(stderr,
            "Error: could not set Core Audio input callback (status %d)\n",
            (int)status);
    AudioComponentInstanceDispose(state.audio_unit);
    exit(EXIT_FAILURE);
  }
  ...
  status = AudioUnitSetProperty(
      state.audio_unit, kAudioUnitProperty_MaximumFramesPerSlice,
