...
static int recordCallback(const void *inputBuffer, void *outputBuffer,
  ...
  if (inputBuffer == NULL)
    write_to_cava_input_buffers(framesToCalc * audio->channels, silence_ptr,
                                audio);
  else
...
void *input_portaudio(void *audiodata) {
  ...
  if (!strcmp(audio->source, "list")) {
    if (numOfDevices < 0) {
      fprintf(
          stderr,
          "Error: portaudio was unable to find a audio device! Code: 0x%x\n",
          numOfDevices);
      exit(EXIT_FAILURE);
    }
  ...
  data.recordedSamples =
      (SAMPLE *)malloc(2 * data.maxFrameIndex * sizeof(SAMPLE));
  ...
  err = Pa_OpenStream(&stream, &inputParameters, NULL, sampleRate,
                      audio->input_buffer_size / audio->channels, paClipOff,
                      recordCallback, &data);
  if (err != paNoError) {
    fprintf(stderr,
