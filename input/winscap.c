...
static void make_stereo_waveformatex(const WAVEFORMATEX *orig,
                                     WAVEFORMATEX *stereo) {
  *stereo = *orig;
...
static void downmix_to_stereo_s16(const int16_t *src, int16_t *dst,
...
static void downmix_to_stereo_s24(const uint8_t *src, int16_t *dst,
                                  size_t frames, int channels) {
  size_t i;
  for (i = 0; i < frames; ++i) {
    int left = 0, right = 0;
...
static void downmix_to_stereo_s32(const int32_t *src, int16_t *dst,
                                  size_t frames, int channels) {
  size_t i;
  for (i = 0; i < frames; ++i) {
...
static void downmix_to_stereo_f32(const float *src, int16_t *dst, size_t frames,
                                  int channels) {
  size_t i;
  for (i = 0; i < frames; ++i) {
...
DeviceChangeNotification_AddRef(IMMNotificationClient *This);
ULONG STDMETHODCALLTYPE
DeviceChangeNotification_Release(IMMNotificationClient *This);
...
void DeviceChangeNotification_Init(DeviceChangeNotification *self,
                                   volatile BOOL *changed, HANDLE hEvent) {
...
void write_silent_frame(struct audio_data *audio, IAudioCaptureClient *pCapture,
  ...
  write_to_cava_input_buffers(numFramesAvailable * silent_channels,
                              (unsigned char *)silence, audio);
...
void process_multichannel(UINT32 numFramesAvailable, const WAVEFORMATEX format,
                          const void *pData, struct audio_data *audio,
                          IAudioCaptureClient *pCapture, UINT32 packetLength,
...
void get_audio_device_by_name(IMMDeviceEnumerator *pEnumerator,
                              const char *device_name, IMMDevice **ppDevice,
                              EDataFlow *pFlow) {
  IMMDeviceCollection *pDeviceCollection = NULL;
  HRESULT hr = pEnumerator->lpVtbl->EnumAudioEndpoints(
      pEnumerator, *pFlow, DEVICE_STATE_ACTIVE, &pDeviceCollection);
  if (FAILED(hr)) {
  ...
  for (UINT i = 0; i < deviceCount; i++) {
    ...
    hr =
        pCurrDevice->lpVtbl->OpenPropertyStore(pCurrDevice, STGM_READ, &pProps);
    if (FAILED(hr)) {
      fwprintf(stderr,
               L"Failed to open property store for device %d: 0x%08lx\n", i,
               hr);
...
void input_winscap(void *data) {
  ...
  static const GUID CLSID_MMDeviceEnumerator = {
      0xBCDE0395, 0xE52F, 0x467C, 0x8E, 0x3D, 0xC4,
      0x57,       0x92,   0x91,   0x69, 0x2E};
  static const GUID IID_IMMDeviceEnumerator = {0xA95664D2, 0x9614, 0x4F35, 0xA7,
                                               0x46,       0xDE,   0x8D,   0xB6,
                                               0x36,       0x17,   0xE6};
  static const GUID IID_IAudioClient = {0x1CB9AD4C, 0xDBFA, 0x4c32, 0xB1,
                                        0x78,       0xC2,   0xF5,   0x68,
                                        0xA7,       0x03,   0xB2};
  ...
  DeviceChangeNotification_Init(&deviceChangeNotification, &deviceChanged,
                                hEvent);
  ...
  while (!audio->terminate) {
    ...
    // fallback to default device
      ...
      pEnumerator->lpVtbl->GetDefaultAudioEndpoint(pEnumerator, eRender,
                                                   eConsole, &pDevice);
    }
    IAudioClient *pClient = NULL;
    pDevice->lpVtbl->Activate(pDevice, &IID_IAudioClient, CLSCTX_ALL, 0,
                              (void **)&pClient);
    ...
    HRESULT hrInit =
        pClient->lpVtbl->Initialize(pClient, AUDCLNT_SHAREMODE_SHARED, strFlags,
    ...
    if (FAILED(hrInit)) {
      ...
      WaitForSingleObject(
          hEvent, INFINITE); // may freeze, need to test on special device
      continue;
    }
    ...
    pClient->lpVtbl->GetService(pClient, &IID_IAudioCaptureClient,
                                (void **)&pCapture);
    ...
    while (!audio->terminate) {
      ...
      while (packetLength) {
        ...
        pCapture->lpVtbl->GetBuffer(pCapture, &pData, &numFramesAvailable,
                                    &flags, 0, 0);
        ...
        if (format.nChannels > 2) {
          process_multichannel(numFramesAvailable, format, pData, audio,
