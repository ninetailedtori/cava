...
#define WCHAR_SIZE                                                             \
  4 // wchar is 4 bytes on linux but only 2 on windows, we need at least 3
...
const wchar_t *left_barchars[] = {L"\u2588", L"\u258F", L"\u258E", L"\u258D",
                                  L"\u258C", L"\u258B", L"\u258A", L"\u2589"};
const wchar_t *right_barchars[] = {L"\u2588",     L"\u2595",    L"\U0001FB87",
                                   L"\U0001FB88", L"\u2590",    L"\U0001FB89",
                                   L"\U0001FB8A", L"\U0001FB8B"};
...
struct colors parse_color(char *color_string) {
  struct colors color;
  sscanf(++color_string, "%02hx%02hx%02hx", &color.rgb[0], &color.rgb[1],
         &color.rgb[2]);
  return color;
}
...
int change_color(int color_idx, int buf_idx, int tty) {
  if (tty) {
    return snprintf(ttyframe_buffer + buf_idx, ttybuf_length - buf_idx,
                    "\033[38;2;%d;%d;%dm", gradient_colors[color_idx].rgb[0],
                    gradient_colors[color_idx].rgb[1],
                    gradient_colors[color_idx].rgb[2]);
  } else {
    return swprintf(frame_buffer + buf_idx, buf_length - buf_idx,
                    L"\033[38;2;%d;%d;%dm", gradient_colors[color_idx].rgb[0],
                    gradient_colors[color_idx].rgb[1],
                    gradient_colors[color_idx].rgb[2]);
  }
}
...
int init_terminal_noncurses(
    int tty, char *const fg_color_string, char *const bg_color_string, int col,
    int bgcol, int vertical_gradient, int gradient_count,
  ...
  if (col == 38) {
    ...
    printf("\033[38;2;%d;%d;%dm", fg_color.rgb[0], fg_color.rgb[1],
           fg_color.rgb[2]);
  ...
  if (bgcol != 0) {
    ...
    if (bgcol == 48) {
      ...
      printf("\033[48;2;%d;%d;%dm", bg_color.rgb[0], bg_color.rgb[1],
             bg_color.rgb[2]);
    } else {
      printf("\033[%dm", bgcol);
    }
  ...
  if (vertical_gradient) {
    ...
    vertical_gradient_colors[gradient_size - 1] =
        gradient_color_defs[gradient_count - 1];
  ...
  if (horizontal_gradient) {
    ...
    for (int i = 0; i < horizontal_gradient_count; i++) {
      horizontal_gradient_color_defs[i] =
          parse_color(horizontal_gradient_color_strings[i]);
    }
    ...
    float rest = number_of_bars / (float)(horizontal_gradient_count - 1) -
                 individual_size;
  ...
  if (horizontal_gradient && vertical_gradient) {
    gradient_colors = (struct colors *)malloc((number_of_bars * gradient_size) *
...
int draw_terminal_noncurses(int bars[], int previous_frame[],
                            enum orientation orientation, void *config) {
      ...
      for (int i = 0; i < number_of_bars; i++) {
        ...
        if ((current_cell < 1 && prev_cell < 1) ||
          ...
          if (tty) {
            ...
            if (same_bar > 0) {
              cx += snprintf(
                  ttyframe_buffer + cx, ttybuf_length - cx, "\033[%dC",
            ...
            if (current_cell > 0) {
              ...
              if (vertical_gradient && horizontal_gradient) {
                if (orientation == ORIENT_BOTTOM) {
                  cx +=
                      change_color(current_line * number_of_bars + i, cx, tty);
                } else if (orientation == ORIENT_TOP) {
                  cx += change_color(
  ...
  if (orientation == ORIENT_LEFT || orientation == ORIENT_RIGHT) {
    ...
    // "vertical_gradient" is horizontal and "horizontal vertical_gradient" is
    // vertical tty not implemented, lacks special unicode characters
          ...
          if (orientation == ORIENT_LEFT) {
            ...
            if (bars[i] > previous_frame[i]) {
              ...
              if (fragment != 0) {
                ...
                if (vertical_gradient && horizontal_gradient) {
                  cx +=
                      change_color(i + number_of_bars * (color_x_pos), cx, tty);
                }
                cx += swprintf(frame_buffer + cx, buf_length - cx,
                               left_barchars[fragment]);
            ...
            if (bars[i] > previous_frame[i]) {
              ...
              if (fragment != 0) {
                ...
                if (vertical_gradient && horizontal_gradient) {
                  cx += change_color(i + number_of_bars * (width - x_pos), cx,
                                     tty);
                }
                x_pos++;
                cx += swprintf(frame_buffer + cx, buf_length - cx,
