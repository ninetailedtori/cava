...
void init_sdl_surface(int *w, int *h, char *const fg_color_string,
                      char *const bg_color_string, int gradient,
                      int gradient_count, char **gradient_color_strings) {
  SDL_GetWindowSize(gWindow, w, h);

  parse_color(bg_color_string, &bg_color);
  SDL_SetRenderDrawColor(gRenderer, fg_color.RGB[0], fg_color.RGB[1],
                         fg_color.RGB[2], 0xFF);
  ...
  if (gradient) {
    ...
    int lines = *h;
    gradient_colors_sdl =
        (struct colors *)malloc((lines * 2) * sizeof(struct colors));
...
int draw_sdl(int bars_count, int bar_width, int bar_spacing, int remainder,
             int height, const int bars[], int previous_frame[], int frame_time,
             enum orientation orientation, int gradient) {
  ...
  if (update) {
    SDL_SetRenderDrawColor(gRenderer, bg_color.RGB[0], bg_color.RGB[1],
                           bg_color.RGB[2], 0xFF);
    SDL_RenderClear(gRenderer);
    if (gradient) {
