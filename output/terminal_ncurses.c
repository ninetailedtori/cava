...
const wchar_t *bar_heights_bottom[] = {L"\u2581", L"\u2582", L"\u2583",
                                       L"\u2584", L"\u2585", L"\u2586",
                                       L"\u2587", L"\u2588"};
const wchar_t *bar_heights_top[] = {L"\u2594",     L"\U0001FB82", L"\U0001FB83",
                                    L"\u2580",     L"\U0001FB84", L"\U0001FB85",
                                    L"\U0001FB86", L"\u2588"};
const wchar_t *bar_heights_left[] = {L"\u258F", L"\u258E", L"\u258D",
                                     L"\u258C", L"\u258B", L"\u258A",
                                     L"\u2589", L"\u2588"};
const wchar_t *bar_heights_right[] = {
    L"\u2595",     L"\U0001FB87", L"\U0001FB88", L"\u2590",
    L"\U0001FB89", L"\U0001FB8A", L"\U0001FB8B", L"\u2588"};
const wchar_t **bar_heights[] = {bar_heights_bottom, bar_heights_top,
                                 bar_heights_left, bar_heights_right};
...
#define COLORS_STRUCT_NORMALIZE(X)                                             \
  CURSES_COLOR_COEFFICIENT(X.R), CURSES_COLOR_COEFFICIENT(X.G),                \
      CURSES_COLOR_COEFFICIENT(X.B)
...
static void get_screen_coords(int line, int col, int max_value,
                              enum orientation orientation, int *x, int *y) {
  switch (orientation) {
  case ORIENT_LEFT:
    *x = line;
    *y = col;
...
void init_terminal_ncurses(char *const fg_color_string,
                           char *const bg_color_string, int predef_fg_color,
                           int predef_bg_color, int gradient,
                           int gradient_count, char **gradient_colors,
                           int *width, int *lines) {
  initscr();
  curs_set(0);
  timeout(0);
  ...
  bg_color_number =
      change_color_definition(0, bg_color_string, predef_bg_color);

  if (!gradient) {

    NCURSES_COLOR_T fg_color_number;
    fg_color_number =
        change_color_definition(1, fg_color_string, predef_fg_color);
    ...
    for (int i = 0; i < gradient_count; i++) {
      int col = (i + 1) * 2 - 2;
      sscanf(gradient_colors[i] + 1, "%02hx%02hx%02hx", &rgb[col][0],
             &rgb[col][1], &rgb[col][2]);
    }
    ...
    for (int i = 0; i < gradient_count - 1; i++) {
      ...
      for (int j = 0; j < individual_size; j++) {
        ...
        change_color_definition(color_pair_number, next_color,
                                color_pair_number);
    ...
    while (left < gradient_size) {
      sprintf(next_color, "#%02x%02x%02x", rgb[col][0], rgb[col][1],
              rgb[col][2]);
...
int draw_terminal_ncurses(int is_tty, int dimension_value, int dimension_bar,
                          int bars_count, int bar_width, int bar_spacing,
                          int rest, const int bars[], int previous_frame[],
  ...
  for (int bar = 0; bar < bars_count; bar++) {
    max_update_value =
        max(max_update_value, max(bars[bar], previous_frame[bar]));
  }
