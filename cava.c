...
float *monstercat_filter(float *bars, int number_of_bars, int waves,
...
\n";
  ...
  while (1) {
    ...
    if (p.live_config) {
      has_config_state =
          get_file_state(configPath, &config_mtime, &config_size);
    }
    ...
    if ((output_mode == OUTPUT_NCURSES || output_mode == OUTPUT_NONCURSES) &&
      ...
      if (inAtty) {
        ...
        if (p.orientation != ORIENT_BOTTOM) {
          cleanup();
          fprintf(stderr,
                  "only default bottom orientation is supported in tty\n");
          exit(EXIT_FAILURE);
        }
      }
    ...
    audio.cava_buffer_size =
        16384; // this is the size at rates of 44100 or 48k, will be
    ...
    case INPUT_PORTAUDIO:
      ...
      if (!strcmp(audio.source, "list")) {
        ...
        thr_id =
            pthread_create(&p_thread, NULL, input_portaudio, (void *)&audio);
      }
      break;
#endif
#ifdef COREAUDIO
    case INPUT_COREAUDIO:
      ...
      if (!strcmp(audio.source, "list")) {
        ...
        thr_id = pthread_create(&p_thread, NULL, input_coreaudio_tap,
                                (void *)&audio);
#endif
      } else {
        thr_id =
            pthread_create(&p_thread, NULL, input_coreaudio, (void *)&audio);
      }
      break;
#endif
#ifdef PIPEWIRE
    ...
    while (true) {
      ...
      if (timeout_counter > 5000) {
        cleanup();
        fprintf(stderr,
                "could not get rate and/or format, problems with audio thread? "
                "quitting...\n");
        exit(EXIT_FAILURE);
      }
    }
    ...
    if (output_mode == OUTPUT_SDL_GLSL) {
      init_sdl_glsl_window(p.sdl_width, p.sdl_height, p.sdl_x, p.sdl_y,
                           p.sdl_full_screen, p.vertex_shader,
                           p.fragment_shader);
      height = p.sdl_height;
      width = p.sdl_width;
      *dimension_value = 1;
    ...
    while (!reloadConf) { // jumping back to this loop means that you resized
      ...
      case OUTPUT_NCURSES:
        init_terminal_ncurses(p.color, p.bcolor, p.col, p.bgcol, p.gradient,
                              p.gradient_count, p.gradient_colors, &width,
                              &lines);
        if (p.xaxis != NONE)
          lines--;
        height = lines;
      ...
      case OUTPUT_SDL:
        init_sdl_surface(&width, &height, p.color, p.bcolor, p.gradient,
      ...
      case OUTPUT_NORITAKE:
        if (strcmp(p.raw_target, "/dev/stdout") != 0) {
          ...
          hFile = CreateNamedPipeA(
              pipePath, PIPE_ACCESS_OUTBOUND | FILE_FLAG_OVERLAPPED,
      ...
      if (p.fixedbars) {
        ...
        if (number_of_bars * p.bar_width + p.fixedbars * p.bar_spacing -
                p.bar_spacing >
            width) {
          cleanup();
          fprintf(
              stderr,
              "window is too narrow for number of bars set, maximum is %d\n",
        ...
        if (number_of_bars < output_channels) {
          ...
          fprintf(
              stderr,
              "fixed number of bars must be at least 1 with mono output and "
        ...
        number_of_bars =
            (*dimension_bar + p.bar_spacing) / (p.bar_width + p.bar_spacing);

        if (output_mode == OUTPUT_SDL_GLSL) {
          if (number_of_bars > 512)
            number_of_bars =
                512; // can't have more than 512 bars in glsl due to shader
                     // program implementation limitations
        } else {
          if (number_of_bars / output_channels > 512)
            number_of_bars =
                512 * output_channels; // can't have more than 512 bars on
      ...
      if (output_mode == OUTPUT_NONCURSES) {
        init_terminal_noncurses(
            inAtty, p.color, p.bcolor, p.col, p.bgcol, p.gradient,
      ...
      int raw_number_of_bars =
          (number_of_bars / output_channels) * audio_channels;
      if (p.waveform) {
      ...
      if (p.userEQ_enabled && (number_of_bars / output_channels > 0)) {
        userEQ_keys_to_bars_ratio =
            (double)(((double)p.userEQ_keys) /
                     ((double)(number_of_bars / output_channels)));
      }

      struct cava_plan *plan = cava_init(
          number_of_bars / output_channels, audio.rate, audio.channels,
          p.autosens, p.noise_reduction, p.lower_cut_off, p.upper_cut_off);
      ...
      if (plan->input_buffer_size != audio.cava_buffer_size) {
        ...
        free(audio.cava_in);
        audio.cava_in =
            (double *)malloc(audio.cava_buffer_size * sizeof(double));
        memset(audio.cava_in, 0, sizeof(double) * audio.cava_buffer_size);
        pthread_mutex_unlock(&audio.lock);
      }

      bars_left =
          (float *)malloc(number_of_bars / output_channels * sizeof(float));
      bars_right =
          (float *)malloc(number_of_bars / output_channels * sizeof(float));
      ...
      memset(cava_out, 0,
             sizeof(double) * number_of_bars * audio.channels /
                 output_channels);
        ...
        for (int n = 0; n < number_of_bars; n++) {
          if (p.stereo) {
            if (n < number_of_bars / 2)
              cut_off_frequency =
                  plan->cut_off_frequency[number_of_bars / 2 - 1 - n];
            else
              cut_off_frequency =
                  plan->cut_off_frequency[n - number_of_bars / 2];
          } else {
          ...
          if (output_mode == OUTPUT_NCURSES) {
            ...
            if (cut_off_frequency < 1000)
              mvprintw(lines, n * (p.bar_width + p.bar_spacing) + remainder,
                       "%-4d", freq_floor);
      ...
      // on sdl we use SDL_delay instead of our own sleep and it is in whole
      // milliseconds, so we need to adjust a little
...
// general: keyboard controls
        ...
        if (!high_framerate) {
          ...
          if (audio.samples_counter >
              audio.input_buffer_size + samples_to_use) {
        ...
        if (!p.waveform) {
          if (audio_channels == 2) {
            for (int n = 0; n < number_of_bars / output_channels; n++) {
              if (p.userEQ_enabled)
                cava_out[n] *= p.userEQ[(int)floor(((double)n) *
                                                   userEQ_keys_to_bars_ratio)];
              bars_left[n] = cava_out[n];
            }
            for (int n = 0; n < number_of_bars / output_channels; n++) {
              if (p.userEQ_enabled)
                cava_out[n + number_of_bars / output_channels] *= p.userEQ[(
                    int)floor(((double)n) * userEQ_keys_to_bars_ratio)];
        ...
        for (int n = 0; n < number_of_bars; n++) {
          ...
          if (output_mode == OUTPUT_SDL_GLSL)
            bars[n] = bars_raw[n] *
                      1000; // values are 0-1, only used to check for changes
...
#ifdef SDL_GLSL
        case OUTPUT_SDL_GLSL:
          rc = draw_sdl_glsl(number_of_bars, bars_raw, previous_bars_raw,
          ...
          if (p.orientation == ORIENT_SPLIT_H ||
            ...
            if (p.split_stereo) {
              ...
              for (int i = 0; i < number_of_bars / 2; i++) {
                ...
                right_previous_frame[i] =
                    previous_frame[i + number_of_bars / 2];
              }
              if (p.orientation == ORIENT_SPLIT_H) {
              ...
              if (p.orientation == ORIENT_SPLIT_V) {
                rc = draw_terminal_noncurses(right_bars, right_previous_frame,
                                             ORIENT_LEFT, &p);
                rc = draw_terminal_noncurses(bars, previous_frame, ORIENT_RIGHT,
                                             &p);
              }
              ...
              if (p.orientation == ORIENT_SPLIT_H) {
                ...
                rc = draw_terminal_noncurses(bars, previous_frame, ORIENT_LEFT,
                                             &p);
                rc = draw_terminal_noncurses(bars, previous_frame, ORIENT_RIGHT,
                                             &p);
              }
            ...
            rc = draw_terminal_noncurses(bars, previous_frame, p.orientation,
                                         &p);
          }
          break;
        case OUTPUT_NCURSES:
          ...
          rc = draw_terminal_ncurses(
              inAtty, *dimension_value / 8, *dimension_bar, number_of_bars,
              p.bar_width, p.bar_spacing, remainder, bars, previous_frame,
              p.gradient, p.xaxis, p.orientation);
          break;
#endif
        case OUTPUT_RAW:
#ifndef _WIN32
        ...
        case OUTPUT_NORITAKE:
#ifndef _WIN32
          rc = print_ntk_out(number_of_bars, fp, p.bit_format, p.bar_width,
        ...
        if (output_mode != OUTPUT_SDL && output_mode != OUTPUT_SDL_GLSL) {
          ...
          elapsedTimens = (t2.tv_sec - t1.tv_sec) * 1000000000.0 +
                          (t2.tv_nsec - t1.tv_nsec);

          int sleep_time_ns =
              frame_time_ns - (int)elapsedTimens - (int)wakeupTimens;
          ...
          if (sleep_time_ns > 1) {
            ...
            double actualSleeptime = (t3.tv_sec - t2.tv_sec) * 1000000000.0 +
    ...
    if (should_quit && signal_received == 0) {
      if (p.zero_test && total_bar_height > 0) {
        fprintf(
            stderr,
            "Test mode: expected total bar height to be zero, but was: %d\n",
            total_bar_height);
        return EXIT_FAILURE;
