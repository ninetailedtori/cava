...
void main() {
  ...
  struct timespec framerate_timer = {.tv_sec = 0,
                                     .tv_nsec = frame_time_msec * 1e6};
#else
  ...
  printf("planning visualization with %d bars per channel, %d rate, %d "
         ...
         bars_per_channel, rate, channels, noise_reduction, low_cut_off,
         high_cut_off);

  struct cava_plan *plan =
      cava_init(bars_per_channel, rate, channels, 1, noise_reduction,
  ...
  for (int k = 0; k < 300; k++) {
    ...
    for (int n = 0; n < buffer_size / 2; n++) {
      cava_in[n * 2] =
          sin(2 * PI * 200 / rate * (n + (k * buffer_size / 2))) * 20000;
      cava_in[n * 2 + 1] =
          sin(2 * PI * 2000 / rate * (n + (k * buffer_size / 2))) * 20000;
    }
  ...
  int bp_ok = 1;
  for (int i = 0; i < bars_per_channel; i++) {
    if (cava_out[i] > blueprint_200MHz[i] * 1.02 ||
        cava_out[i] < blueprint_200MHz[i] * 0.98)
      bp_ok = 0;
  }
  for (int i = 0; i < bars_per_channel; i++) {
