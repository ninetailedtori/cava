  ...
  for (int n = 0; n < p->number_of_bars + 1; n++) {
    ...
    if (n > 0) {
      if (!first_bar) {
        ...
        // the bass and calculating new cut off frequencies
...
void cava_execute(double *cava_in, int new_samples, double *cava_out,
  ...
  int silence = 1;
  if (new_samples > 0) {
    // process: approximate actual framerate. This will be off by +10% at 60
    // fps, but should be good enough for the autosens and smoothing algorithms
    // to be adjusted accordingly if framerate is a lot more or less.
    p->framerate -= p->framerate / 64.0;
    p->framerate += (double)(p->rate * p->frame_skip) /
                    (new_samples / p->audio_channels) / 64.0;
    p->frame_skip = 1;
  ...
  for (int n = 0; n < p->number_of_bars * p->audio_channels; n++) {
    ...
    cava_out[n] =
        p->cava_mem[n] * p->noise_reduction / integral_mod + cava_out[n];
